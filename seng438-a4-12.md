...
Since only one action is taken in some situations, such as adding to or removing from the wish list, it is not feasible to test it with various data sets. However, by using faulty usernames (email addresses) and passwords, scenarios like the login example could be tested.
